<?php
	...
	function userLogin($login, $pass, $db) {
		//pripravim si dotaz
		try {
			$query = $db->prepare("SELECT COUNT(*) FROM uziv WHERE login = ? AND heslo = ?");
		...
		if($query->fetchColumn(0) == 1) {
			...
			try {
				$query = $db->prepare("SELECT ID FROM uziv WHERE login = ? AND heslo = ?");
			} catch (PDOException $e) {
				die($e->getMessage());
			}
	...
	function getUserLogin($id, $db) {
		//pripravim si dotaz
		try {
			$query = $db->prepare("SELECT login FROM uziv WHERE ID = ?");
		} catch (PDOException $e) {
			die($e->getMessage());
		}
		//pole s parametry
